...
import { Flame, Leaf, Plus, Minus, Trash2, Sparkles, Sunrise, ArrowRight, Pencil } from "lucide-react";
...
const THEMES = {
  ...
  breakfast: {
    ...
    icon: Sunrise,
    bg: "#FBEADD",
    bgGradient: "linear-gradient(180deg, #FDF1E4 0%, #F7D9B8 100%)",
    surface: "#FFF6EC",
    border: "#F0D9BE",
    ink: "#4A342A",
    muted: "#9C8171",
...
// ---------- Goal configs: calorie adjustment + macro ratio, applied to computed TDEE ----------
const GOAL_CONFIG = {
...
const ACTIVITY_LEVELS = {
  ...
  moderate: { label: "Moderate", desc: "3-5 days/week", factor: 1.55 },
  active: { label: "Active", desc: "6-7 days/week", factor: 1.725 },
};

// Mifflin-St Jeor BMR -> TDEE -> goal-adjusted calories -> macro grams
...
// ---------- Mock restaurant data (swap for real DB-backed data later) ----------
const RESTAURANTS = {
  mcdonalds: {
    name: "McDonald's",
    items: [
      { id: "m1", name: "10 pc Chicken McNuggets", cal: 410, p: 23, c: 26, f: 24, period: "standard" },
      ...
      { id: "m4", name: "Egg McMuffin", cal: 310, p: 17, c: 30, f: 13, period: "breakfast" },
      { id: "m5", name: "Sausage Burrito", cal: 300, p: 12, c: 26, f: 16, period: "breakfast" },
      { id: "m6", name: "Hotcakes (3, no syrup/butter)", cal: 350, p: 8, c: 60, f: 8, period: "breakfast" },
      { id: "m7", name: "Hash Browns", cal: 150, p: 1, c: 15, f: 9, period: "breakfast" },
    ],
  },
  chipotle: {
    name: "Chipotle",
    items: [
      { id: "c1", name: "Chicken Burrito Bowl (double chicken)", cal: 620, p: 68, c: 40, f: 18, period: "standard" },
      { id: "c2", name: "Steak Bowl w/ rice & beans", cal: 640, p: 40, c: 66, f: 22, period: "standard" },
      { id: "c3", name: "Barbacoa Tacos (3)", cal: 540, p: 33, c: 42, f: 26, period: "standard" },
      { id: "c4", name: "Chicken Salad, no dressing", cal: 350, p: 44, c: 15, f: 12, period: "standard" },
    ],
  },
  tacobell: {
    name: "Taco Bell",
    items: [
      { id: "t1", name: "Power Menu Bowl - Chicken", cal: 470, p: 26, c: 46, f: 20, period: "standard" },
      { id: "t2", name: "Cheesy Gordita Crunch", cal: 500, p: 18, c: 41, f: 29, period: "standard" },
      { id: "t3", name: "3 Crunchy Tacos Supreme", cal: 540, p: 24, c: 39, f: 33, period: "standard" },
      { id: "t4", name: "Bean Burrito", cal: 350, p: 13, c: 54, f: 9, period: "standard" },
      { id: "t5", name: "Breakfast Crunchwrap", cal: 610, p: 20, c: 53, f: 36, period: "breakfast" },
      { id: "t6", name: "Hash Brown", cal: 160, p: 1, c: 15, f: 10, period: "breakfast" },
    ],
  },
};
...
// ---------- Growing plant meter (signature element) ----------
    ...
    <div className="flex flex-col items-center gap-2 w-full">
      <div className="relative h-32 w-10 flex items-end justify-center">
        <div
          ...
          style={{ height: `${stemHeight}px`, backgroundColor: color }}
        />
        {Array.from({ length: leafCount }).map((_, i) => (
          <div
            ...
            style={{
              ...
              transform: i % 2 === 0 ? "rotate(-25deg)" : "rotate(25deg) scaleX(-1)",
            }}
          >
      ...
      <div className="text-center">
        <div className="text-xs font-medium tracking-wide" style={{ color: ink }}>
          {label}
        </div>
...
const ACTIVITY_EXAMPLES = {
  ...
  active: "Hard training 6-7 days a week, or physically demanding work (construction, delivery, etc.)",
};
...
function Onboarding({ onComplete }) {
  ...
  const [heightValue, setHeightValue] = useState(68);
  const [weightLb, setWeightLb] = useState(170);
  const [activity, setActivity] = useState("moderate");
  const [hoveredActivity, setHoveredActivity] = useState(null);
  const theme = THEMES.standard;

  const heightIn = heightUnit === "cm" ? heightValue / 2.54 : heightValue;
          ...
          <div className="flex flex-wrap gap-2 mb-2">
            {Object.entries(GOAL_CONFIG).map(([key, g]) => (
              <button
                ...
                style={{
                  backgroundColor: goalKey === key ? theme.primary : theme.bg,
                  color: goalKey === key ? theme.surface : theme.ink,
                }}
              >
                {g.label}
              </button>
        ...
        <div className="rounded-3xl p-6 mb-5" style={{ backgroundColor: theme.surface, border: `1px solid ${theme.border}` }}>
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            <button
              ...
              style={{ backgroundColor: sex === "male" ? theme.primary : theme.bg, color: sex === "male" ? theme.surface : theme.ink }}
            >
              Male
            </button>
            <button
              ...
              className="py-2 rounded-xl text-sm font-medium"
              style={{ backgroundColor: sex === "female" ? theme.primary : theme.bg, color: sex === "female" ? theme.surface : theme.ink }}
            >
              Female
            </button>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 mb-3">
            ...
            <label className="flex flex-col gap-1">
              <span className="text-xs flex items-center justify-between" style={{ color: theme.muted }}>
                Height ({heightUnit})
                <button type="button" onClick={toggleHeightUnit} className="text-[10px] underline" style={{ color: theme.primary }}>
                  switch to {heightUnit === "in" ? "cm" : "in"}
                </button>
              </span>
              <input type="number" value={heightValue} onChange={(e) => setHeightValue(Number(e.target.value))}
                className="rounded-lg px-2 py-2 text-sm" style={{ backgroundColor: theme.bg, color: theme.ink, border: `1px solid ${theme.border}` }} />
            </label>
            <label className="flex flex-col gap-1">
          ...
          <div className="grid grid-cols-3 gap-2 mt-1">
            {Object.entries(ACTIVITY_LEVELS).map(([key, a]) => (
              <div key={key} className="relative" onMouseEnter={() => setHoveredActivity(key)} onMouseLeave={() => setHoveredActivity(null)}>
                ...
                {hoveredActivity === key && (
                  <div
                    className="absolute z-10 left-1/2 -translate-x-1/2 top-full mt-2 w-40 rounded-xl px-3 py-2 text-[11px] leading-snug shadow-lg"
...
// ---------- Main app ----------
function App({ profile, goalKey, onEditProfile }) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm transition-colors duration-500" style={{ color: theme.muted }}>
              {GOAL_CONFIG[goalKey].label} plan · {targets.calories} cal target
            </p>
          </div>
        </div>
...
export default function MacroLoadout() {
  const [session, setSession] = useState(null); // null = show onboarding

  if (!session) {
    return <Onboarding onComplete={setSession} />;
  }
  return <App profile={session.profile} goalKey={session.goalKey} onEditProfile={() => setSession(null)} />;
}
